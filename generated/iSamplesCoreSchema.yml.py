# Auto generated from iSamplesCoreSchema.yml by pythongen.py version: 0.9.0
# Generation date: 2022-10-21T15:13:32
...
class PhysicalSampleRecord(YAMLRoot):
    ...
    label: str = None
    sampleidentifier: str = None
    $schema: Optional[str] = None
    @id: Optional[str] = None
    description: Optional[str] = None
    hasContextCategory: Optional[Union[str, List[str]]] = empty_list()
    hasMaterialCategory: Optional[Union[Union[str, "Materialtype"], List[Union[str, "Materialtype"]]]] = empty_list()
    hasSpecimenCategory: Optional[Union[str, List[str]]] = empty_list()
    informalClassification: Optional[Union[str, List[str]]] = empty_list()
    keywords: Optional[Union[str, List[str]]] = empty_list()
    producedBy: Optional[Union[dict, "SamplingEvent"]] = None
    registrant: Optional[str] = None
    samplingPurpose: Optional[str] = None
    curation: Optional[Union[dict, "Curation"]] = None
    relatedResource: Optional[Union[dict, "SampleRelation"]] = None
    authorizedBy: Optional[str] = None
    compliesWith: Optional[str] = None
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        if self._is_empty(self.sampleidentifier):
            self.MissingRequiredField("sampleidentifier")
        if not isinstance(self.sampleidentifier, str):
            self.sampleidentifier = str(self.sampleidentifier)
        ...
        if not isinstance(self.hasContextCategory, list):
            self.hasContextCategory = [self.hasContextCategory] if self.hasContextCategory is not None else []
        self.hasContextCategory = [v if isinstance(v, str) else str(v) for v in self.hasContextCategory]

        if not isinstance(self.hasMaterialCategory, list):
            self.hasMaterialCategory = [self.hasMaterialCategory] if self.hasMaterialCategory is not None else []
        self.hasMaterialCategory = [v if isinstance(v, Materialtype) else Materialtype(v) for v in self.hasMaterialCategory]

        if not isinstance(self.hasSpecimenCategory, list):
            self.hasSpecimenCategory = [self.hasSpecimenCategory] if self.hasSpecimenCategory is not None else []
        self.hasSpecimenCategory = [v if isinstance(v, str) else str(v) for v in self.hasSpecimenCategory]

        if not isinstance(self.informalClassification, list):
            self.informalClassification = [self.informalClassification] if self.informalClassification is not None else []
        self.informalClassification = [v if isinstance(v, str) else str(v) for v in self.informalClassification]

        if not isinstance(self.keywords, list):
            self.keywords = [self.keywords] if self.keywords is not None else []
        self.keywords = [v if isinstance(v, str) else str(v) for v in self.keywords]

        if self.producedBy is not None and not isinstance(self.producedBy, SamplingEvent):
            self.producedBy = SamplingEvent(**as_dict(self.producedBy))

        if self.registrant is not None and not isinstance(self.registrant, str):
            self.registrant = str(self.registrant)

        if self.samplingPurpose is not None and not isinstance(self.samplingPurpose, str):
            self.samplingPurpose = str(self.samplingPurpose)

        if self.curation is not None and not isinstance(self.curation, Curation):
            self.curation = Curation(**as_dict(self.curation))

        if self.relatedResource is not None and not isinstance(self.relatedResource, SampleRelation):
            self.relatedResource = SampleRelation(**as_dict(self.relatedResource))

        if self.authorizedBy is not None and not isinstance(self.authorizedBy, str):
            self.authorizedBy = str(self.authorizedBy)

        if self.compliesWith is not None and not isinstance(self.compliesWith, str):
            self.compliesWith = str(self.compliesWith)
...
class SamplingSite(YAMLRoot):
    ...
    placeName: Optional[Union[str, List[str]]] = empty_list()
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        if not isinstance(self.placeName, list):
            self.placeName = [self.placeName] if self.placeName is not None else []
        self.placeName = [v if isinstance(v, str) else str(v) for v in self.placeName]
...
class SamplingEvent(YAMLRoot):
    ...
    label: Optional[str] = None
    description: Optional[str] = None
    hasFeatureOfInterest: Optional[str] = None
    responsibility: Optional[Union[str, List[str]]] = empty_list()
    resultTime: Optional[str] = None
    samplingSite: Optional[Union[dict, SamplingSite]] = None
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        if self.hasFeatureOfInterest is not None and not isinstance(self.hasFeatureOfInterest, str):
            self.hasFeatureOfInterest = str(self.hasFeatureOfInterest)
        ...
        if self.resultTime is not None and not isinstance(self.resultTime, str):
            self.resultTime = str(self.resultTime)

        if self.samplingSite is not None and not isinstance(self.samplingSite, SamplingSite):
            self.samplingSite = SamplingSite(**as_dict(self.samplingSite))
...
class Curation(YAMLRoot):
    ...
    class_class_uri: ClassVar[URIRef] = ISAM.Curation
    class_class_curie: ClassVar[str] = "isam:Curation"
    class_name: ClassVar[str] = "Curation"
    class_model_uri: ClassVar[URIRef] = ISAM.Curation

    label: Optional[str] = None
    description: Optional[str] = None
    accessConstraints: Optional[Union[str, List[str]]] = empty_list()
    curationLocation: Optional[str] = None
    responsibility: Optional[Union[str, List[str]]] = empty_list()
    ...
    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        ...
        if not isinstance(self.accessConstraints, list):
            self.accessConstraints = [self.accessConstraints] if self.accessConstraints is not None else []
        self.accessConstraints = [v if isinstance(v, str) else str(v) for v in self.accessConstraints]

        if self.curationLocation is not None and not isinstance(self.curationLocation, str):
            self.curationLocation = str(self.curationLocation)
...
slots.@id = Slot(uri=ISAM['@id'], name="@id", curie=ISAM.curie('@id'),
                   model_uri=ISAM['@id'], domain=None, range=Optional[str])

slots.$schema = Slot(uri=ISAM['$schema'], name="$schema", curie=ISAM.curie('$schema'),
                   model_uri=ISAM['$schema'], domain=None, range=Optional[str])

slots.authorizedBy = Slot(uri=ISAM.authorizedBy, name="authorizedBy", curie=ISAM.curie('authorizedBy'),
                   model_uri=ISAM.authorizedBy, domain=None, range=Optional[str])

slots.compliesWith = Slot(uri=ISAM.compliesWith, name="compliesWith", curie=ISAM.curie('compliesWith'),
                   model_uri=ISAM.compliesWith, domain=None, range=Optional[str])

slots.relatedResource = Slot(uri=ISAM.relatedResource, name="relatedResource", curie=ISAM.curie('relatedResource'),
                   model_uri=ISAM.relatedResource, domain=None, range=Optional[Union[dict, SampleRelation]])

slots.relationship = Slot(uri=ISAM.relationship, name="relationship", curie=ISAM.curie('relationship'),
                   model_uri=ISAM.relationship, domain=None, range=Optional[str])

slots.target = Slot(uri=ISAM.target, name="target", curie=ISAM.curie('target'),
                   model_uri=ISAM.target, domain=None, range=Optional[str])

slots.accessConstraints = Slot(uri=ISAM.accessConstraints, name="accessConstraints", curie=ISAM.curie('accessConstraints'),
                   model_uri=ISAM.accessConstraints, domain=None, range=Optional[Union[str, List[str]]])

slots.curation = Slot(uri=ISAM.curation, name="curation", curie=ISAM.curie('curation'),
                   model_uri=ISAM.curation, domain=None, range=Optional[Union[dict, Curation]])

slots.curationLocation = Slot(uri=ISAM.curationLocation, name="curationLocation", curie=ISAM.curie('curationLocation'),
                   model_uri=ISAM.curationLocation, domain=None, range=Optional[str])

slots.label = Slot(uri=ISAM.label, name="label", curie=ISAM.curie('label'),
                   model_uri=ISAM.label, domain=None, range=Optional[str])

slots.description = Slot(uri=ISAM.description, name="description", curie=ISAM.curie('description'),
                   model_uri=ISAM.description, domain=None, range=Optional[str])

slots.informalClassification = Slot(uri=ISAM.informalClassification, name="informalClassification", curie=ISAM.curie('informalClassification'),
                   model_uri=ISAM.informalClassification, domain=PhysicalSampleRecord, range=Optional[Union[str, List[str]]])

slots.registrant = Slot(uri=ISAM.registrant, name="registrant", curie=ISAM.curie('registrant'),
                   model_uri=ISAM.registrant, domain=PhysicalSampleRecord, range=Optional[str])

slots.sampleidentifier = Slot(uri=ISAM.sampleidentifier, name="sampleidentifier", curie=ISAM.curie('sampleidentifier'),
                   model_uri=ISAM.sampleidentifier, domain=PhysicalSampleRecord, range=Optional[str])

slots.samplingPurpose = Slot(uri=ISAM.samplingPurpose, name="samplingPurpose", curie=ISAM.curie('samplingPurpose'),
                   model_uri=ISAM.samplingPurpose, domain=PhysicalSampleRecord, range=Optional[str])

slots.keywords = Slot(uri=ISAM.keywords, name="keywords", curie=ISAM.curie('keywords'),
                   model_uri=ISAM.keywords, domain=PhysicalSampleRecord, range=Optional[Union[str, List[str]]])

slots.hasContextCategory = Slot(uri=ISAM.hasContextCategory, name="hasContextCategory", curie=ISAM.curie('hasContextCategory'),
                   model_uri=ISAM.hasContextCategory, domain=PhysicalSampleRecord, range=Optional[Union[str, List[str]]])

slots.hasMaterialCategory = Slot(uri=ISAM.hasMaterialCategory, name="hasMaterialCategory", curie=ISAM.curie('hasMaterialCategory'),
                   model_uri=ISAM.hasMaterialCategory, domain=PhysicalSampleRecord, range=Optional[Union[Union[str, "Materialtype"], List[Union[str, "Materialtype"]]]])

slots.hasSpecimenCategory = Slot(uri=ISAM.hasSpecimenCategory, name="hasSpecimenCategory", curie=ISAM.curie('hasSpecimenCategory'),
                   model_uri=ISAM.hasSpecimenCategory, domain=PhysicalSampleRecord, range=Optional[Union[str, List[str]]])

slots.producedBy = Slot(uri=ISAM.producedBy, name="producedBy", curie=ISAM.curie('producedBy'),
                   model_uri=ISAM.producedBy, domain=PhysicalSampleRecord, range=Optional[Union[dict, "SamplingEvent"]])

slots.location = Slot(uri=ISAM.location, name="location", curie=ISAM.curie('location'),
                   model_uri=ISAM.location, domain=SamplingSite, range=Optional[Union[dict, "GeospatialDDCoordLocation"]])

slots.placeName = Slot(uri=ISAM.placeName, name="placeName", curie=ISAM.curie('placeName'),
                   model_uri=ISAM.placeName, domain=SamplingSite, range=Optional[Union[str, List[str]]])

slots.hasFeatureOfInterest = Slot(uri=ISAM.hasFeatureOfInterest, name="hasFeatureOfInterest", curie=ISAM.curie('hasFeatureOfInterest'),
                   model_uri=ISAM.hasFeatureOfInterest, domain=SamplingEvent, range=Optional[str])

slots.responsibility = Slot(uri=ISAM.responsibility, name="responsibility", curie=ISAM.curie('responsibility'),
                   model_uri=ISAM.responsibility, domain=None, range=Optional[Union[str, List[str]]])

slots.resultTime = Slot(uri=ISAM.resultTime, name="resultTime", curie=ISAM.curie('resultTime'),
                   model_uri=ISAM.resultTime, domain=SamplingEvent, range=Optional[str])

slots.samplingSite = Slot(uri=ISAM.samplingSite, name="samplingSite", curie=ISAM.curie('samplingSite'),
                   model_uri=ISAM.samplingSite, domain=SamplingEvent, range=Optional[Union[dict, SamplingSite]])

slots.elevation = Slot(uri=ISAM.elevation, name="elevation", curie=ISAM.curie('elevation'),
                   model_uri=ISAM.elevation, domain=GeospatialDDCoordLocation, range=Optional[str])

slots.latitude = Slot(uri=ISAM.latitude, name="latitude", curie=ISAM.curie('latitude'),
                   model_uri=ISAM.latitude, domain=GeospatialDDCoordLocation, range=Optional[Decimal])

slots.longitude = Slot(uri=ISAM.longitude, name="longitude", curie=ISAM.curie('longitude'),
                   model_uri=ISAM.longitude, domain=GeospatialDDCoordLocation, range=Optional[Decimal])

slots.PhysicalSampleRecord_label = Slot(uri=ISAM.label, name="PhysicalSampleRecord_label", curie=ISAM.curie('label'),
                   model_uri=ISAM.PhysicalSampleRecord_label, domain=PhysicalSampleRecord, range=str)

slots.PhysicalSampleRecord_sampleidentifier = Slot(uri=ISAM.sampleidentifier, name="PhysicalSampleRecord_sampleidentifier", curie=ISAM.curie('sampleidentifier'),
                   model_uri=ISAM.PhysicalSampleRecord_sampleidentifier, domain=PhysicalSampleRecord, range=str)